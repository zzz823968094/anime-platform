...
public class CrawlerService2 {
    ...
    /**
     * 增量爬取所有分类
     * @Async注解使该方法异步执行,调用方立即返回,不会阻塞HTTP请求线程
     */
    ...
    private boolean crawlDetail(String url, String sourceId, int type) throws Exception {
        ...
        // 番剧入库或更新
        AnimeTable existing = animeTableMapper.selectOne(
                new LambdaQueryWrapper<AnimeTable>().eq(AnimeTable::getVodId, Integer.valueOf(sourceId))
        );
        ...
        if (existing == null) {
            AnimeTable anime = new AnimeTable();
            anime.setVodName(title);
            anime.setVodPic(coverImage);
            anime.setVodContent(synopsis);
            anime.setVodId(Integer.valueOf(sourceId));
            anime.setTypeId(type);
            anime.setVodStatus(status == 2 ? (byte) 1 : (byte) 0);
            anime.setVodSerial(String.valueOf(episodes.size()));
            anime.setVodHits(0);
            if (rating > 0) anime.setVodScore(BigDecimal.valueOf(rating));
            animeTableMapper.insert(anime);
            animeId = anime.getId();
            changed = true;
            log.info("[爬虫] 新番剧: {} (type={}, status={})", title, type, status);
        } else {
            animeId = existing.getId();
            changed = existing.getVodSerial() == null
                    || !existing.getVodSerial().equals(String.valueOf(episodes.size()))
                    || existing.getVodStatus() != (status == 2 ? (byte) 1 : (byte) 0);
            existing.setVodSerial(String.valueOf(episodes.size()));
            existing.setVodStatus(status == 2 ? (byte) 1 : (byte) 0);
            if (existing.getTypeId() == null) {
                existing.setTypeId(type);
            }
            if ((existing.getVodContent() == null || existing.getVodContent().isBlank())
                    && !synopsis.isBlank()) {
                existing.setVodContent(synopsis);
                changed = true;
            }
            if (rating > 0 && (existing.getVodScore() == null
                    || existing.getVodScore().doubleValue() == 0)) {
                existing.setVodScore(BigDecimal.valueOf(rating));
            }
            animeTableMapper.updateById(existing);
        }
